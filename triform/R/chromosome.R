...
test.chr <- function(chipfiles,
  ...
  for(direction in DIRECTIONS){
    ...
    # problem: surL uses both samples
    ...
    # thoughts:
    # summarized such as surL, surR trivial;
    # what to do about those individual observations?
    #   signs1, signs2, ok, etc?
    #   only used to check which ok, right? can be done in loop....
  ...
  PEAKS[[direction]] <<- list(IRanges(),IRanges(),IRanges())
  PEAK.INFO[[direction]] <<- list(NULL,NULL,NULL)
  PEAK.INFO[["regions"]] <<- list(NULL,NULL,NULL)

  neg.cvg <- CENTER.CVG[[1]]
  pos.cvg <- CENTER.CVG[[2]]
  ...
  for (i in 1:3) {
    ...
    info1 <- PEAK.INFO[[1]][[i]][ov[,1],]
    info2 <- PEAK.INFO[[2]][[i]][ov[,2],]
